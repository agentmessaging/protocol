...
|-----------|----------|-------------|
| `grant_type` | Yes | Must be `urn:amp:agent-card` |
| `agent_card` | Yes | Base64url-encoded signed Agent Card JSON (see [02 - Identity](02-identity.md#agent-card-format)) |
...
&agent_card=eyJhbXBfYWdlbnRfY2FyZCI6IjEuMCIsImFkZHJlc3MiOi...
...
Decode the `agent_card` parameter from base64url and parse as JSON. Verify the card conforms to the Agent Card format defined in [02 - Identity](02-identity.md#agent-card-format).

### 2. Verify the Agent Card Signature

Follow the Agent Card verification procedure from [02 - Identity](02-identity.md#verifying-an-agent-card):
...
### 3. Check Agent Card Expiration

Verify that the card's `expires_at` timestamp is in the future. Expired cards MUST be rejected with `invalid_grant`.
...
An Agent Card alone is insufficient to obtain a token. The proof of possession requires the agent's Ed25519 private key to generate a valid signature. An attacker who intercepts or copies an Agent Card cannot authenticate without the corresponding private key.
...
### Agent Cards (Section 02)

This section builds directly on the Agent Card format defined in [02 - Identity](02-identity.md#agent-card-portable-identity). The Agent Card serves as the identity assertion in the token exchange. All Agent Card signing and verification rules from Section 02 apply.

### Key Rotation (Section 07)

When an agent rotates its Ed25519 key, it MUST obtain a new Agent Card signed with the new key. Old Agent Cards become invalid for token exchange once the key is revoked. See [07 - Security](07-security.md#key-rotation).
...
4. **Support key rotation gracefully.** When a new Agent Card arrives with the same address but a different key, verify the card signature and update the stored public key.
